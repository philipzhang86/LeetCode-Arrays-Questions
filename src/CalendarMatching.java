...
public class CalendarMatching {
    ...
    private static List<Meeting> updateCalendar(List<StringMeeting> calendar, StringMeeting dailyBounds){
        List<StringMeeting> update = new ArrayList<>();
        update.add(new StringMeeting("0:00", dailyBounds.start));
        update.addAll(calendar);
        update.add(new StringMeeting(dailyBounds.end, "23:59"));
        List<Meeting> calendarInMinutes = new ArrayList<>();
        for (int i = 0; i<update.size(); i++) {
            calendarInMinutes.add(
                    new Meeting(
                            timeToMinutes(update.get(i).start),
                            timeToMinutes(update.get(i).end)));
        }
    ...
    private static List<StringMeeting> getMatchingAvailabilities(
        ...
        for(int i = 0; i < matchingAvailabilities.size(); i++){
            matchingAvailabilitiesInHours.add(
                    new StringMeeting(minutesToTime(matchingAvailabilities.get(i).start),
    ...
    public static void main(String[] args) {
        String[][] s1 = {{"9:00", "10:30"}, {"12:00", "13:00"},{"16:00", "18:00"}};
        List<StringMeeting> calendar1 = new ArrayList<>();
        for(String[] next : s1){
            String start = next[0];
            String end = next[1];
            calendar1.add(new StringMeeting(start,end));
        }

        String[][] s2 = {{"10:00", "11:30"}, {"12:30", "14:30"},{"14:30", "15:00"}, {"16:00", "17:00"}};
        List<StringMeeting> calendar2 = new ArrayList<>();
        for(String[] next : s2){
            String start = next[0];
            String end = next[1];
            calendar2.add(new StringMeeting(start,end));
        }
